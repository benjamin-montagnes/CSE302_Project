...
class Block:
    ...
    def __eq__(self, other):
        return self.label==other.label and self.body==other.body and self.jumps==other.jumps

    def __repr__(self):
        rep = self.label+'\n'
        for line in self.body:
            rep += str(line)+'\n'
        for line in self.jumps:
            rep += str(line)+'\n'
        return rep
...
class CFG:
    ...
    def __init__(self, proc_name, lab_entry, blocks, t_args):
        """
    ...
    def write_dot(self, tacfile):
        ...
        with open(dotfile, 'w') as f:
            ...
            for bl in self._blockmap.values():
                node_label = str(bl).replace('\n', '\\l')
                print(f'{bl.label[1:]}[shape="box",fontname="monospace",fontsize=8,label="{node_label}"];', file=f)
...
_enders = re.compile(r'jmp|jz|jnz|jl|jle|ret')
_jumps  = re.compile(r'jmp|jz|jnz|jl|jle')
_jcc    = re.compile(r'jz|jnz|jl|jle')
...
def infer(tac_proc):
    ...
    while cur < len(tac_proc.body):
        ...
        while cur < len(tac_proc.body):
            ...
            cur += 1
        blocks.append(bl)
    return CFG(tac_proc.name, lab_entry, blocks, tac_proc.t_args)
